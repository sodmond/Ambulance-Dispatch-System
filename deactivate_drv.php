...
<title>::.. Deactivate a Driver</title>
...
<div class="content">
<div id="box"><br />
<span style="font:1.5em Tahoma, Geneva, sans-serif; color:#900;">Deactivate Driver</span>
<hr />
...
<p align="justify" style="color:#F00;"><b>Note:</b> <span style="font-size:0.9em">Drivers listed in the menu below are Active</span></p>
...
<?php
$sql = mysqli_query($conn, 'SELECT Driver_id FROM drivers_status WHERE Status="Active"');
while($drv = mysqli_fetch_array($sql)){
echo '<option value="'.$drv['Driver_id'].'">'.$drv['Driver_id'].'</option>';
}
?>
</select></p>
<p><input type="submit" value="Check OUT" class="btn" /></p>
</form>
...
<?php
		...
		if(empty($_POST['drv'])){
			...
			header('Location: deactivate_drv.php');
		}
		else{
			$driver_id = $_POST['drv'];
			$query = mysqli_query($conn, 'SELECT Req_id FROM dispatch_status WHERE Driver_id="'.$driver_id.'" ORDER BY Date DESC LIMIT 0,1');
			$row = mysqli_fetch_array($query);
			$sql0 = mysqli_query($conn, 'INSERT INTO dispatch_status(Driver_id, Admin_id, Req_id, Status)
										VALUES("'.$driver_id.'", "'.$_SESSION['admin'].'", "'.$row['Req_id'].'", "Inactive")');
			$sql1 = mysqli_query($conn, 'UPDATE drivers_status SET Status="Inactive" WHERE Driver_id="'.$driver_id.'"');
			if($sql0 && $sql1){
				unset($_SESSION['req_id']);
				$_SESSION['suces'] = 'Driver Checked OUT Succesfully';
				header('Location: deactivate_drv.php');
			}
			else{
				$_SESSION['error'] = 'Driver Checked OUT failed';
				header('Location: deactivate_drv.php');
			}
		}
	}
